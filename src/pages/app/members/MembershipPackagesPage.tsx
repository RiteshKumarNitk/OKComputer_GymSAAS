  ...
  const columns: Column<Membership>[] = [
    {
      key: "id",
      label: "ID",
      render: (plan) => <span className="font-mono text-sm text-slate-500">{plan.id.substring(0, 5)}</span>,
    },
    {
      key: "name",
      label: "Package Name",
    ...
    },
    {
      key: "duration",
      label: "Duration",
      render: (plan) => <span className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-tighter">{Math.ceil((plan.durationDays ?? 0) / 30)} Months</span>,
    },
    {
      key: "sessions",
      label: "Sessions",
      render: (plan) => <span className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-tighter">{Math.ceil((plan.durationDays ?? 0) / 30) * 30}</span>,
    },
    {
      key: "price",
      label: "Price",
      render: (plan) => <span className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-tighter">₹{(plan.priceCents ?? 0) / 100}</span>,
    },
    {
      key: "active",
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 -mx-4 px-4 md:mx-0 md:px-0 no-scrollbar">
        {categories.map((cat, idx) => (
          <Card key={cat.name} className={`${idx === 0 ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20' : 'bg-white dark:bg-slate-900 text-slate-400'} border-none min-w-[200px] flex-1 rounded-xl group hover:scale-[1.02] transition-all`}>
      ...
      <Dialog open={showPlanForm} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[425px] rounded-xl border-none shadow-2xl">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-slate-900">{selectedPlan ? "Edit Plan" : "Add New Plan"}</DialogTitle>
      ...
      <ConfirmDialog
        ...
        confirmLabel={deletePlanMutation.isPending ? "Deleting..." : "Delete"}
