...
import { auditLogExtension } from "../lib/auditLogExtension.js"
 ...
 * lib/prismaTenantScope.ts), and automatically writes an AuditLog entry for
 * mutations on Member/Payment/UserProfile/Tenant (see
 * lib/auditLogExtension.ts). Exported under the same name as before so no
 ...
 */
export const prisma = basePrisma.$extends(tenantScopeExtension()).$extends(auditLogExtension())
...
export function authenticate(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    tenantContextStorage.run({ tenantId: req.tenantId ?? null, role: req.role ?? null, userId: req.userId ?? null }, next)
  } catch (err) {
